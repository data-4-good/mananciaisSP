...
dados<-read.csv2("mananciais_att.csv")
str(dados)
head(dados)
...
cant<-subset(dados,subset = manancial=="Cantareira")
cant$data.<-as.Date(cant$data.,format = "%Y-%m-%d")
...
pluviom<-insertRow(pluviom,newrow = c(0,"agosto",0.398),r = 56)
...
pluviom$varvolume[142]<-pluviom$varvolume[142]-0.106
pluviom$varvolume[137]<-pluviom$varvolume[137]-0.185
...
```{r results="hide"}
tiete<-subset(dados,subset = manancial=="AltoTiete")
tiete$data.<-as.Date(tiete$data.,format = "%Y-%m-%d")
...
```{r}
which(tiete$aumentovolume>0.05)
tiete[4364:4366,]
modelo<-with(tiete[-c(4365),],lm(aumentovolume ~ pluviometria.do.dia))
modelo
summary(modelo)$r.squared 
...
pluviom$varvolume[144]<-pluviom$varvolume[144]-0.065
